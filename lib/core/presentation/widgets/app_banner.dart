...
class AppBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(color: style.backgroundColor, borderRadius: BorderRadius.circular(12)),
      padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 16).copyWith(bottom: actions != null ? 0 : 10),
      margin: const EdgeInsets.symmetric(vertical: 5),
      child: Material(
        color: Colors.transparent,
        child: InkWell(
          onTap: onTap != null || bannerRemovePolicies.contains(const AppBannerRemovePolicy.click())
              ? () {
...
extension on AppBannerStyle {
  Color get backgroundColor => switch (this) {
    AppBannerStyle.success => const Color(0xFF0F2A1F),
    AppBannerStyle.error => const Color(0xFF2D0F0F),
    AppBannerStyle.info => const Color(0xFF0F1F2D),
    AppBannerStyle.warning => const Color(0xFF2D2510),
  };

  Color get titleTextColor => AppColorPalette.white;

  Color get textColor => AppColorPalette.white;
}
    ...
    if (mounted) {
      setState(() {
        ...
        if (withEffect) {
          ...
          _bannerListKey.currentState?.removeItem(index, (_, _) => Container(height: 0), duration: Duration.zero);
        }
        _processQueue();
      });
    }
  ...
  Future<void> _processQueue() async {
    await _controller._lock.synchronized(() {
      ...
      while (_bannerQueue.isNotEmpty && _banners.length < policyAmount) {
        ...
        final seconds = banner.bannerRemovePolicies
            .whereType<TimedBannerRemovePolicy>()
            .map((TimedBannerRemovePolicy policy) => policy.duration.inSeconds)
            .firstOrNull;

        if (seconds != null) {
          _timedRemoveMap[banner] = DateTime.now()
              .add(Duration(milliseconds: millis, seconds: seconds))
              ...
              itemBuilder: (BuildContext ctx, int index, Animation<double> animation) {
                ...
                return FadeTransition(
                  ...
                  child: SizeTransition(
                    ...
                    child: banner.bannerRemovePolicies.contains(const AppBannerRemovePolicy.slide())
                        ? Dismissible(
